...
Centralenheten är häjrnan av systemet. Enhetens huvudsakliga uppgiften är att hålla en ständigt kommunikation mellan centralenheten och inkopplade enheterna. Kommunikationen kommer att omfatta ACK-, larm-, och konfigurationsmeddelanden. Ack meddelanden är ett sätt att se till att kopplade enheterna verkligen mottagar meddelande från centralenheten och vice versa. Detta innebär att vid kommunikation av två enheterna, enhet A skickar till B, förväntas då ack-meddelande från enhet B, vilket signilerar i sin tur att meddelanden har nått destinationen. Däremot inte alla typer av meddelande skickar ack, exempelvis meddelandtyp ``LIFESIGN'' och ``ack'' kräver ingen ack-respons när de skickas till andra enheterna, detta för att undvicka skapa oändlig loop (Figur 5). En ständig kommunikation innebär att centralenheten förvänta sig att samtiliga inkopplade enheterna skickar kontineurligt ifrån sig meddelande med typ "Life Sign" i en förbestämd tid intervall av 500ms. Skulle en enhet inte skicka en "Life Sign" meddelande inom 500 ms då får den en ytterliggare 500 ms att skicka, annars slås på alarm av centralenheten. 
Dessutom ska centralenheten hantera all kommunikation mellan användaren och systemet genom USART. För att användaren ska få tillgång till systemet ska det korrekta lösenordet skrivas in. Användaren får en lista med 9 val att välja på. Till exempel ska användaren kunna, för varje dörr, slå på eller av det centrala larmet, ändra tröskelvärdet, låsa eller låsa upp dörren. Utöver det ska användaren också kunna ändra tröskelvärdet för avståndsmätaren och ser status på alla enheterna som är kopplade till centralenheten.
